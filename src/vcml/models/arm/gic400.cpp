...
void gic400::cpuif::write_bpr(u32 val) {
    abpr = extract(val, 0, 3); // read only first 3 bits, store copy in ABPR
    bpr = abpr;
}
...
void gic400::cpuif::write_eoir(u32 val) {
    ...
    if (irq >= m_parent->get_irq_num()) {
        log_warn("(eoi) invalid irq %zu ignored", irq);
        return;
    }
    ...
    for (int cpu = 0; cpu < m_cpu_num; cpu++) {
        ...
        if (!virt) {
            if (best_prio < cpuif.pmr.bank(cpu)) {
                log_debug("setting irq %zu pending on cpuif %u", best_irq,
                          cpu);
