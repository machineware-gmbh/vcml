...
void backend_tcp::iothread() {
    ...
    while (m_running) {
        try {
            ...
            if (client >= 0) {
                can_frame frame{};
                deserialize(frame, [&](u8* buffer, size_t buflen) {
                    m_socket.recv(client, buffer, buflen);
                });
...
void backend_tcp::send_to_host(const can_frame& frame) {
    ...
    for (int client : clients) {
        try {
            serialize(frame, [&](const u8* buffer, size_t buflen) {
                m_socket.send(client, buffer, buflen);
            });
        } catch (...) {
            // nothing to do
        }
