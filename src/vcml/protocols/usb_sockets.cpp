...
void usb_host::usb_reset(int ep) {
    // to be overloaded
}

void usb_host::usb_transport(const usb_target_socket& socket, usb_packet& p) {
    usb_transport(p);
}

void usb_host::usb_transport(usb_packet& p) {
    // to be overloaded
}
...
usb_initiator_socket::usb_initiator_socket(const char* nm, address_space as):
    ...
    m_host(hierarchy_search<usb_host>()),
    m_transport(this) {
    bind(m_transport);
...
usb_speed usb_target_stub::usb_get_speed() {
