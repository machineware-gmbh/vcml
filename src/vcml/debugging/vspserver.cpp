...
string vspserver::handle_rmwp(const string& cmd) {
    ...
    watchpoint_type type = (watchpoint_type)from_string<u32>(args[2]);
    if (type != VSP_WATCHPOINT_READ && type != VSP_WATCHPOINT_WRITE &&
        type != VSP_WATCHPOINT_ACCESS) {
        return mkstr("E,invalid watchpoint type '%u'", type);
    }

    vcml_access wp_type = VCML_ACCESS_NONE;
    switch (type) {
    case VSP_WATCHPOINT_READ:
        wp_type = VCML_ACCESS_READ;
        break;
    case VSP_WATCHPOINT_WRITE:
        wp_type = VCML_ACCESS_WRITE;
        break;
    case VSP_WATCHPOINT_ACCESS:
        wp_type = VCML_ACCESS_READ_WRITE;
        break;
    default:
        VCML_ERROR("invalid watchpoint type");
    }
    if (!tgt.remove_watchpoint(it->second, wp_type, this))
